...
# Load a local .env (e.g. GEMINI_API_KEY) if present. Optional — lab still runs without it.
try:
...
SYSTEM_PROMPT_PATH = APP_DIR / 'data' / 'prompt_templates' / 'system_prompt.txt'

_DEFAULT_SYSTEM_PROMPT = 'You are an AIoT decision assistant. Use only the provided context. Return JSON only.'

def load_system_prompt() -> str:
    """Read the editable system prompt from disk so it can be tuned without code changes."""
    try:
...
# Expert-intended "correct" risk per timeline step (C9). One label per timeline entry,
# used by eval_harness.py to measure each tier's accuracy. Kept separate from SCENARIOS
# so the scenario data stays as the model's input only.
GROUND_TRUTH: Dict[str, List[str]] = {
    'lab_overcrowded_high_co2': ['LOW', 'MEDIUM', 'HIGH', 'HIGH', 'CRITICAL'],
    'fire_alarm_conflict':      ['LOW', 'LOW', 'MEDIUM', 'MEDIUM', 'CRITICAL'],
    'fall_or_bending_ambiguity':['LOW', 'MEDIUM', 'MEDIUM', 'HIGH', 'CRITICAL'],
    'ppe_danger_zone':          ['LOW', 'LOW', 'CRITICAL', 'CRITICAL'],
    'greenhouse_leaf_disease_risk': ['LOW', 'MEDIUM', 'MEDIUM', 'HIGH'],
}
...
def sensor_ai_decision(sensors: Dict[str, Any], scenario_id: str) -> Dict[str, Any]:
    ...
    if scenario_id == 'fire_alarm_conflict':
        ...
        if fconf > 0.45 and smoke < 10 and gas < 10 and parse_bool(sensors.get('projector_on')):
            # Conflicting evidence: keep it as a reviewable MEDIUM, do not over-escalate to HIGH/CRITICAL.
            risk = 'MEDIUM'
...
# Gemini structured-output schema (OpenAPI subset Gemini accepts: uppercase types,
# no additionalProperties). Mirrors OUTPUT_SCHEMA / validate_llm_output so the model
# can only return a valid decision.
...
# JSON Schema cho Ollama structured outputs (lowercase types). Ép model nhỏ trả đúng
# 7 trường — mirror OUTPUT_SCHEMA / validate_llm_output.
OLLAMA_DECISION_SCHEMA = {
    'type': 'object',
    'properties': {
        'situation_summary': {'type': 'string'},
        'risk_level': {'type': 'string', 'enum': ['LOW', 'MEDIUM', 'HIGH', 'CRITICAL']},
...
def call_ollama(context: Dict[str, Any], model: Optional[str] = None) -> Dict[str, Any]:
    ...
    payload = {
        ...
        'stream': False,
        'format': OLLAMA_DECISION_SCHEMA,  # structured output: ép đúng schema quyết định
        # think=false tắt "thinking" của model qwen3/gemma3 → nhanh hơn nhiều cho lab.
        # Đặt OLLAMA_THINK=true nếu muốn bật lại reasoning sâu.
        'think': parse_bool(os.getenv('OLLAMA_THINK', 'false')),
...
def _gemini_config() -> tuple[str, str, str]:
    """Resolve (api_key, model, base_url) for Gemini, raising if no key is configured."""
    if requests is None:
        raise RuntimeError('requests is not installed')
...
def call_gemini(context: Dict[str, Any], model: Optional[str] = None,
                image: Optional[Dict[str, str]] = None) -> tuple[Dict[str, Any], Optional[Dict[str, int]]]:
    """Call Google Gemini for the 'api' mode via the REST API (structured JSON output).

    Uses responseMimeType=application/json + responseSchema so the reply is guaranteed
    to be valid JSON matching GEMINI_DECISION_SCHEMA. If `image` ({'mime_type','data'(base64)})
    is given, it is attached as a real vision input (Gemini is multimodal).

    Returns (decision_dict, usage_dict) where usage_dict has prompt/output/total tokens.
    Requires GEMINI_API_KEY; any failure raises and reason_with_llm() falls back to the mock.
    """
    api_key, default_model, base = _gemini_config()
    model = model or default_model
    parts: List[Dict[str, Any]] = [{'text': build_prompt(context)}]
    if image is not None:
        parts.append({'text': 'Một ảnh quan sát được đính kèm. Hãy mô tả ngắn gọn những gì thấy '
                              'trong ảnh và đưa nó vào evidence_used khi ra quyết định.'})
    ...
    payload = {
        ...
        'generationConfig': {
            ...
            # thinkingBudget=0 disables Gemini 2.5 "thinking" → snappy lab latency.
            # Raise it (e.g. 1024) for deeper reasoning at the cost of speed.
    ...
    r = requests.post(
        ...
        json=payload,
        timeout=int(os.getenv('GEMINI_TIMEOUT_SEC', '60')),
    )
    r.raise_for_status()
    data = r.json()
...
def call_gemini_stream(context: Dict[str, Any], model: Optional[str] = None):
    """Stream a plain-language (non-JSON) explanation from Gemini, yielding text chunks.

    Used by /reason-stream for a live "typing" reasoning panel. Separate from the structured
    decision path (no responseSchema) so the text is human-readable as it arrives.
    """
    api_key, default_model, base = _gemini_config()
    model = model or default_model
    prompt = (build_prompt(context) +
              '\nHãy giải thích tình huống và đề xuất hành động an toàn bằng tiếng Việt, '
              'ngắn gọn theo từng ý. KHÔNG trả JSON, chỉ văn xuôi.')
    payload = {
        'systemInstruction': {'parts': [{'text': load_system_prompt()}]},
    ...
    with requests.post(
        f'{base}/models/{model}:streamGenerateContent?alt=sse',
        headers={'x-goog-api-key': api_key, 'Content-Type': 'application/json'},
        json=payload, stream=True, timeout=int(os.getenv('GEMINI_TIMEOUT_SEC', '60')),
    ) as r:
        r.raise_for_status()
...
def reason_with_llm(context: Dict[str, Any], mode: str = 'mock',
    ...
    try:
        if mode == 'mock':
            ...
            # Cloud LLM via Google Gemini (REST, structured JSON). Falls back to mock on any error.
    ...
    write_csv_row(OUTPUT_DIR / 'latency_report.csv', {
        'timestamp': logrow['timestamp'], 'context_id': context['context_id'],
        'scenario_id': context['scenario_id'], 'provider': provider, 'latency_ms': latency_ms,
        'prompt_tokens': (usage or {}).get('prompt_tokens', ''),
        'output_tokens': (usage or {}).get('output_tokens', ''),
        'total_tokens': (usage or {}).get('total_tokens', ''),
    })
    return {'provider': provider, 'latency_ms': latency_ms, 'usage': usage, 'raw_llm_output': raw, 'validated': valid, 'validation_message': validation_msg, 'final_decision': final}
...
@app.get('/scenarios')
def list_scenarios():
    return [{'scenario_id': sid, 'title': s['title'], 'why': s['why_it_is_interesting'], 'timeline_length': len(s['timeline'])} for sid, s in SCENARIOS.items()]
...
@app.get('/compare-models/{scenario_id}')
def compare_models(scenario_id: str, modes: str = 'mock,api'):
    """A3: run several LLM modes in parallel on the same telemetry and compare them
    side by side (provider, latency, token usage, risk, summary)."""
    if scenario_id not in SCENARIOS:
        raise HTTPException(status_code=404, detail='Unknown scenario')
    snap = STATE.snapshot()
    sensors = snap['sensors'] if snap['scenario_id'] == scenario_id else dict(SCENARIOS[scenario_id]['timeline'][0])
    ...
    lock = threading.Lock()
    def run(m: str) -> None:
        ctx = build_context_packet(sensors, scenario_id)
        res = reason_with_llm(ctx, mode=m)
        fd = res['final_decision']
        with lock:
            results[m] = {
                'provider': res['provider'], 'latency_ms': res['latency_ms'],
                'validated': res['validated'], 'usage': res.get('usage'),
                'risk_level': fd['risk_level'], 'need_human_review': fd['need_human_review'],
                'situation_summary': fd['situation_summary'], 'recommended_action': fd['recommended_action'],
            }
    threads = [threading.Thread(target=run, args=(m,)) for m in mode_list]
    for t in threads:
        t.start()
    for t in threads:
        t.join()
    return {'scenario_id': scenario_id, 'telemetry': sensors, 'models': results}

@app.get('/reason-stream/{scenario_id}')
def reason_stream(scenario_id: str, mode: str = 'api'):
    """A2: stream a live plain-language explanation (SSE). mode=api streams from Gemini;
    other modes emit the mock summary in one chunk."""
    if scenario_id not in SCENARIOS:
        raise HTTPException(status_code=404, detail='Unknown scenario')
    snap = STATE.snapshot()
    sensors = snap['sensors'] if snap['scenario_id'] == scenario_id else dict(SCENARIOS[scenario_id]['timeline'][0])
    ...
    def gen():
        try:
            if mode == 'api':
                for piece in call_gemini_stream(context):
                    yield f'data: {json.dumps({"text": piece}, ensure_ascii=True)}\n\n'
            else:
                dec = reason_with_llm(context, mode=mode)['final_decision']
                txt = f"{dec['situation_summary']}\n\nHành động: {dec['recommended_action']}"
                yield f'data: {json.dumps({"text": txt}, ensure_ascii=True)}\n\n'
        except Exception as e:
            dec = mock_llm_reason(context)
            txt = f"[stream lỗi, dùng mock] {dec['situation_summary']}"
            yield f'data: {json.dumps({"text": txt, "error": str(e)}, ensure_ascii=True)}\n\n'
...
async def vision_reason(scenario_id: str, mode: str = 'mock', image: Optional[UploadFile] = File(default=None)):
    """Image-aware endpoint. In mode=api the uploaded image is sent to Gemini (real vision);
    other modes record an image observation in the context (no real VLM)."""
    if scenario_id not in SCENARIOS:
        raise HTTPException(status_code=404, detail='Unknown scenario')
    snap = STATE.snapshot()
    sensors = snap['sensors'] if snap['scenario_id'] == scenario_id else dict(SCENARIOS[scenario_id]['timeline'][0])
    ...
    if image is not None:
        ...
        context['uploaded_image_observation'] = {
            ...
            'note': 'Real vision via Gemini when mode=api; observation-only for mock/local.'
        }
        if mode == 'api' and raw_bytes:
            import base64
            image_payload = {
                'mime_type': image.content_type or 'image/jpeg',
                'data': base64.b64encode(raw_bytes).decode('ascii'),
            }
    return reason_with_llm(context, mode=mode, image=image_payload)
...
def stream_events():
    def gen():
        ...
        while True:
            snap = STATE.snapshot()
            state_str = json.dumps(snap, ensure_ascii=True, sort_keys=True)
            if state_str != last_state:
                last_state = state_str
                yield f'data: {json.dumps({"time": now_iso(), "state": snap}, ensure_ascii=True)}\n\n'
            time.sleep(1)
    return StreamingResponse(gen(), media_type='text/event-stream; charset=utf-8')
