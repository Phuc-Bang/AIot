...
def decision_from_outputs(occupancy_probability: float, anomaly_score: float, co2: float, light: float) -> Dict:
    ...
    if is_anomaly:
        ...
        decision = "CRITICAL_LIGHTING_EMERGENCY"
        command_hint = "lighting_state=EMERGENCY_ON"
        safety_note = "CẢNH BÁO: Có người trong điều kiện thiếu sáng trầm trọng! Đã kích hoạt chế độ đèn khẩn cấp."
