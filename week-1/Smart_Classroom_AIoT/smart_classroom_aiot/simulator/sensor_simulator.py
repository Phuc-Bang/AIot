...
def generate_sensor_data():
    ...
    # 5. Trạng thái AC và Đèn - dùng state nội bộ, được cập nhật qua classroom/control
...
def on_connect(client, userdata, flags, rc):
    if rc == 0:
        ...
        logger.info("Subscribed to classroom/control for state sync")
    else:
        logger.error(f"Connection failed with code {rc}")

def on_message(client, userdata, msg):
    """Nhận lệnh điều khiển và đồng bộ state nội bộ để tránh bị ghi đè."""
    global state
    try:
        payload = json.loads(msg.payload.decode())
        device = payload.get("device")
        command = payload.get("command")
        if device == "ac" and command in ("ON", "OFF"):
            state["ac_state"] = command
            logger.info(f"Control sync: ac_state → {command}")
        elif device == "light" and command in ("ON", "OFF"):
            state["light_state"] = command
            logger.info(f"Control sync: light_state → {command}")
...
def main():
    ...
    logger.info("--------------------------------------------------")
    logger.info(f"SIMULATOR STARTING")
    logger.info(f"Target Broker: {MQTT_BROKER}:{MQTT_PORT}")
    logger.info(f"Target Topic : {MQTT_TOPIC}")
    logger.info("--------------------------------------------------")
    ...
    try:
        logger.info("✅ SUCCESS: Simulator connected and loop started.")
        while True:
            ...
            if result.rc == mqtt.MQTT_ERR_SUCCESS:
                logger.info(f"📤 Sent to {MQTT_TOPIC} | Temp: {data['temperature']}°C")
            else:
